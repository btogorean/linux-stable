...
static int addicmos_power_on(struct device *dev)
{
	...
	pr_err("Entered addicmos_power_on");
...
static int addicmos_power_off(struct device *dev)
{
	...
	pr_err("Entered addicmos_power_off\n");
...
pr_err("Entered addicmos_s_ctrl\n");
				    ...
				    struct v4l2_subdev_pad_config *cfg)
{
	...
	fmt.format.width = addicmos_mode_info_data[1].width;
	fmt.format.height = addicmos_mode_info_data[1].height;
...
{
#if 0
	return regmap_multi_reg_write(addicmos->regmap,
			addicmos->fw_regs,
...
static int addicmos_g_sensor_firmware(struct v4l2_subdev *sd)
{
	...
	__v4l2_ctrl_modify_range(addicmos->set_operating_range,
				 addicmos->set_operating_range->minimum,
				 modes_nr - 1, 1, 0);
...
pr_err("Entered addicmos_mode_firmware_load\n");
...
pr_err("Entered addicmos_probe\n");
	...
	addicmos->set_operating_range = v4l2_ctrl_new_custom(&addicmos->ctrls,
